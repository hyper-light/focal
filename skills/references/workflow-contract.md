...
`ledger.standing` reports which engine serves the selected ledger. On a native ledger the adapter offers the version-2 application tools (including the peer tools `claim.challenge`, `claim.consult`, `claim.correct`, `claim.follow_up`, the composed `claim.lineage` read and the `claim.wait` observer with its `testament` predicate) and exactly four recovery tools; the reserve/seal tools, epoch queries, transfer tools, `claim.progress`, `claim.submit_batch`, `claim.supersede`, `ledger.summary`, `ledger.traverse` and `validator.*` are absent because the owner supplies their guarantees differently. Do not call an absent tool by analogy; report what the catalogue offers.

**Identity.** Every native mutation mints its own durable `n1:` reference before transmission and returns it as `operation_id`; there is no reservation step. To resume an uncertain call, repeat the same tool with the same arguments plus that `operation_id`: identical input resumes the saved frame, different input is refused as `operation_conflict` before any identity is minted. `request.pending` lists unconsumed references (`Committed` or `Pending`), `request.inspect` returns the saved result or the exact pending state (`remote: true` reads the owner's committed outcome by identity), `request.retry` resends the exact frame or returns the committed receipt, and `request.acknowledge` retires a consumed committed result. A refusal is final and typed (`invalid_input`, `unauthorized`, `not_found`, `operation_conflict`, `capacity`); `OutcomeUnknown` means the frame may have committed and only the exact retry settles it. A `capacity` refusal admitted nothing; the reference stays `Pending` for a later retry.

**Documents.** Version-2 inputs name objects by 32-hex IDs and hashes by 64-hex strings, bind the current object revision at compile time, and take payloads inline (`{"type":"text","text":…}` or `{"type":"inline","bytes":[…]}`, bounded at 16 KiB); there is no evidence set, upload or download on the native engine. Frozen vocabularies read back as codes (claim status: `2` posted, `3` received, `7` validating, `8` satisfied, `12` validation incomplete; outcomes: `1` complete, `6` failed; verdicts: `1` pass, `2` fail, `3` incomplete, `4` error) and states as names (`Received`, `Validating`, `Validated`, `ValidationIncomplete`, `Errored`).

**Reads and lists.** `claim.get`, `testament.get`, `artifact.get` and `validation.get` return exact objects at one prefix (a claim with its responses and evaluations; an artifact with its inline payload bytes and provenance). Lists (`claim.list`, `artifact.list`, `validation.list`, `evaluation.list`, `testament.list`, `receipt.list`, `monitor.list`, `event.list`) are bounded scans: keep the filters unchanged across continuations, and treat a page without a cursor as the only end; an empty page with a cursor is not absence.

**Watches.** `watch.open` on a native ledger replays committed native events as schema-2 deltas (`fact.Native` with the exact event record, the coarse lifecycle action and the claim), seeds through exact reads at the pinned snapshot, and acknowledges by delivery identity exactly as on a V1 ledger.
