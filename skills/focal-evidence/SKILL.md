...
description: >-
  ...
  artifacts, transfer payloads, close immutable testaments, submit peer verdicts, or prepare
...
2. Produce the requested material, then call `artifact.submit` with that claim, receipt, evidence set, artifact kind and exact installed schema hash. Use the discovered payload shape. Small text or byte payloads are supported. For larger content, follow the [transfer branch](../references/workflow-contract.md#transfer-actual-payloads): `upload.begin`, `upload.append` and `upload.seal` retain exact transfer identity and bytes across retries. `upload.cancel` ends staging; `artifact.download` retrieves actual payload pages. Completion: the real server returns the immutable content reference before it is supplied to attachment. Schema provisioning remains a separate installed capability.
...
1. As the immutable claim issuer, use `testament.receive` for the exact current closing testament. Completion: the committed acknowledgment is visible; receiving a response does not assert its substantive quality.
...
3. The designated evaluator invokes the pinned tool, skill or code in its own environment. Retain its actual proof as registered artifacts, then use `validation.submit` with that run’s complete fences and exact evidence ID/hash pairs. Completion: the committed verdict records the actual pass, fail, error or incomplete finding. Tool output alone is not a committed verdict, and stale or differently assigned runs confer no standing.
