...
When `ledger.standing` reports the native engine, evidence is bound to manifest slots instead of evidence sets and every payload is inline; follow the [native branch](../references/workflow-contract.md#native-engine).

1. As the current receipt holder (`receipt.acquire` on the `claim`), deliver each requested slot with `artifact.submit` (`claim`, `slot`, `payload`, optional `kind`/`schema_hash`); read back `artifact.get` for the descriptor `content_hash` and the inline payload. Record what failed with `artifact.diagnostic` (`claim`, `reason` `work`, `production`, `structure` or `metadata`, `payload`) and mark a slot you cannot produce with `artifact.fail` (`claim`, `slot`, `diagnostic` citing your own committed `production` diagnostic).
2. Close the cycle with `testament.submit` (`claim`, `summary`, `confidence`, `outcome`, `manifest` of `{slot, artifact: {id, hash}}`, `diagnostics` of `{id, hash}`). A non-`complete` outcome must cite at least one of your own committed `work` diagnostics and is refused before sending without one. Then `testament.post`. Completion: the response is your immutable account; `testament.get` shows its outcome code, manifest, failed work and diagnostics.
3. The issuer receives it with `testament.receive`, reads a delivered output with `artifact.get`, accepts a generated output with `artifact.receive`, or rejects a `structure`/`metadata` failure with `artifact.reject` (`claim`, `artifact`, `reason`, `payload`); a `work` failure is testimony, not a rejection. Survey with `artifact.list` (`producer`, `kind`, `schema`, `input`), `testament.list` (`claim`) and `claim.get`. A slot the frozen manifest lacks is assessed at whole-work entry by the [validation skill](../focal-validation/SKILL.md); the diagnostic bytes stay readable to the claimant and the evaluator through `artifact.get`. `validation.context` composes the evaluator's view.
