...
impl ClaimContent {
    pub fn issuer(&self) -> Option<ParticipantId> {
        self.party(RelationKind::Issuer)
    }
    pub fn subject(&self) -> Option<ParticipantId> {
        self.party(RelationKind::Subject)
    }
    fn party(&self, kind: RelationKind) -> Option<ParticipantId> {
        self.relations
            .iter()
            .find_map(|r| match (r.kind == kind, &r.target) {
                (true, RelationTarget::Participant(p)) => Some(*p),
                _ => None,
            })
    }
    pub fn action(&self) -> Option<ActionType> {
        self.relations.iter().find_map(|r| match r {
            Relation {
                kind: RelationKind::ClaimAction,
                target: RelationTarget::Action(a),
            } => Some(*a),
            _ => None,
        })
    }
    pub fn cause(&self) -> Option<Cause> {
        self.relations.iter().find_map(|r| match r {
            ...
            Relation {
                kind: RelationKind::CausedBy,
                target: RelationTarget::Object(o),
            } if o.kind == ObjectKind::Claim => Some(Cause::Claim(ClaimId(o.id.0))),
            _ => None,
        })
    }
    pub fn dependencies(&self, kind: RelationKind) -> impl Iterator<Item = ClaimId> + '_ {
        self.relations
            ...
            .filter_map(|r| match &r.target {
                RelationTarget::Object(o) if o.kind == ObjectKind::Claim => Some(ClaimId(o.id.0)),
                _ => None,
            })
    }
}
