...
impl StagedManagedMutation {
    ...
    fn validate_basis(&self, view: CoreView<'_>, limits: &Limits) -> Result<(), CoreError> {
        if self.prepared.schema != MANAGED_REQUEST_SCHEMA {
            return Err(CoreError::UnsupportedSchema(self.prepared.schema));
        }
...
impl Core {
    ...
    pub fn stage_managed_pending_bounded(
        ...
        let result = self.stage_managed_recorded(input, view, &recorder);
        if recorder.failed() {
    ...
    pub fn replay_managed_bounded(
        ...
        if prepared.schema != MANAGED_REQUEST_SCHEMA {
            return Err(CoreError::UnsupportedSchema(prepared.schema));
        }
        if prepared.base != self.sequence() {
            return Err(CoreError::StalePreparation);
        }
        let staged = self
            .stage_managed_pending_bounded(&PendingState::new(), &prepared.input, max_bytes)
    ...
    pub fn audit_managed_pending_stage(
        ...
        let replay = self
            .stage_managed_pending_bounded(pending, &staged.prepared.input, max_bytes)
            .map_err(staging_error)?;
    ...
    fn stage_managed_recorded(
        ...
        let command_hash = managed_command_hash(input)?;
