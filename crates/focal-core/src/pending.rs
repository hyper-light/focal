...
impl PendingState {
    ...
    pub fn validate_next(&self, core: &Core, staged: &StagedMutation) -> Result<(), CoreError> {
        execution::Version::from_schema(staged.prepared.schema)?;
...
impl Core {
    pub(crate) fn stage_recorded(
        ...
        // New requests currently admit only the V1 contract. Persist the same
        // selection that produced these rows; replay selects from that record.
        let rules = execution::Version::V1;
