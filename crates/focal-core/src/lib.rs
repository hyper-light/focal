...
impl Core {
    ...
    fn apply_recorded(
        ...
        if prepared.schema != SCHEMA_MAJOR {
        ...
        if prepared.input.ledger != state.ledger()
            || command_hash(&prepared.input)? != prepared.command_hash
        {
        ...
        let (mut draft, outcome, deltas, effects) =
            reduce::execute_on(overlay, &self.limits, &prepared.input, sequence)
    ...
    pub fn decode_checkpoint(bytes: &[u8]) -> Result<Self, CoreError> {
        let tail = bytes.strip_prefix(b"FOCALCP1").ok_or(CoreError::Checksum)?;
        let (checksum, payload) = tail.split_at_checked(32).ok_or(CoreError::Checksum)?;
        if blake3::hash(payload).as_bytes() != checksum {
            return Err(CoreError::Checksum);
        }
        let (schema, core): (u16, Self) = postcard::from_bytes(payload)?;
        if schema != SCHEMA_MAJOR {
            return Err(CoreError::UnsupportedSchema(schema));
        }
        Ok(core)
    }
