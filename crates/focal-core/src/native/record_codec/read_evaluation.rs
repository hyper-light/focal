...
impl Plan {
    ...
    pub(super) fn build(self, allowance: usize) -> Result<(Row, usize), NativeError> {
        if self.heap_bytes() > allowance {
            return Err(NativeError::Capacity("evaluation recovery allowance"));
        }
        let value = OwnedEvaluation::new(self.value)?;
        let actual = value.heap_charge()?;
        if actual > allowance {
            return Err(NativeError::Capacity("evaluation recovery capacity"));
        }
