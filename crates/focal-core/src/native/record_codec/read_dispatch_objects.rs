...
use focal_model::lifecycle::{
    ...
    claim::ClaimState,
    evidence::{Response, ResponseArtifacts, WorkArtifact},
    validation::{AcceptedResult, Declaration, EvaluationState},
};
use focal_model::{ArtifactRef, ValidationId};
use std::cell::Cell;

pub(super) struct Access<'a, O> {
    objects: &'a O,
    lookup: &'a Meter,
    error: Cell<Option<NativeError>>,
}
impl<'a, O: Objects> Access<'a, O> {
    pub(super) fn new(objects: &'a O, lookup: &'a Meter) -> Self {
        Self {
            objects,
            lookup,
            error: Cell::new(None),
    ...
    pub(super) fn ledger(&self) -> LedgerId {
        self.objects.ledger()
    }
    ...
    pub(super) fn claim_dependency(&self, id: ClaimId) -> Result<ClaimDependency<'_>, NativeError> {
        debit(self.lookup, 256)?;
        let dependency = self.objects.claim_dependency(id, self.lookup)?;
        if let ClaimDependency::Retained(claim) = &dependency {
            let binding = claim.binding();
            let policy = claim.acceptance();
            if binding.ledger != self.ledger()
                || binding.object.0 != id.0
                || policy.claim().ledger != binding.ledger
                || policy.claim().object != binding.object
                || policy.claim().content != binding.content
                || policy.issuer() != claim.issuer()
            {
                return Err(invalid());
            }
        }
    ...
    pub(super) fn declaration(&self, id: ValidationId) -> Result<&Declaration, NativeError> {
        let Row::Definition(row) = self.get(Key::Definition(id))? else {
            return Err(invalid());
        };
        let value = row.get().ok_or_else(invalid)?;
        if value.binding().ledger != self.ledger() || value.binding().object.0 != id.0 {
            return Err(invalid());
        }
        Ok(value)
    }
    fn claim(&self, id: ClaimId) -> Result<&ClaimState, NativeError> {
        let Row::Claim(row) = self.get(Key::Claim(id))? else {
            return Err(invalid());
        };
    ...
    fn response(&self, id: TestamentId) -> Result<&Response, NativeError> {
        let Row::Response(row) = self.get(Key::Response(id))? else {
            return Err(invalid());
        };
        let value = row.get().ok_or_else(invalid)?;
        if value.identity().binding.ledger != self.ledger()
            || value.identity().binding.object.0 != id.0
        {
            return Err(invalid());
        }
        Ok(value)
    ...
    fn evaluation(&self, key: EvaluationKey) -> Result<&EvaluationState, NativeError> {
        let Row::Evaluation(row) = self.get(Key::Evaluation(key))? else {
            return Err(invalid());
        };
        let value = row.get().ok_or_else(invalid)?;
        if value.binding().ledger != self.ledger() || EvaluationKey::of(key.claim, value) != key {
            return Err(invalid());
        }
        Ok(value)
    }
    fn artifact(&self, reference: ArtifactRef) -> Result<&ArtifactDescriptor, NativeError> {
        let Row::Artifact(row) = self.get(Key::Artifact(reference.id))? else {
            return Err(invalid());
        };
        let value = row.get().ok_or_else(invalid)?.descriptor();
        if value.ledger() != self.ledger()
            || value.id() != reference.id
            || value.content_hash() != reference.hash
        {
    ...
    fn work(&self, id: ArtifactId) -> Result<&WorkArtifact, NativeError> {
        let Row::Work(row) = self.get(Key::Work(id))? else {
            return Err(invalid());
        };
    ...
    fn result(
        &self,
        key: NativeResultKey,
    ) -> Result<(&AcceptedResult, PublicationPosition), NativeError> {
        let mut found = None;
        ...
        for candidate in [
            ...
            if found.is_some()
                || value.0.ledger() != self.ledger()
                || NativeResultKey::of(*value.0) != key
            {
                return Err(invalid());
            }
            found = Some(value);
    ...
    fn event(&self, at: PublicationPosition) -> Result<NativeEvent, NativeError> {
        let Row::Event(row) = self.get(Key::Event(at.sequence, at.ordinal))? else {
            return Err(invalid());
        };
        debit(self.lookup, 256)?;
...
impl<O: Objects> read_claim::Objects for Access<'_, O> {
    fn declaration(&self, id: ValidationId) -> Result<&Declaration, ContractError> {
        self.contract(self.declaration(id))
    }
    fn response(&self, id: TestamentId) -> Result<&Response, ContractError> {
        self.contract(self.response(id))
    }
    fn evaluation(&self, key: EvaluationKey) -> Result<&EvaluationState, ContractError> {
        self.contract(self.evaluation(key))
    }
}
impl<O: Objects> evidence::Dependencies for Access<'_, O> {
    fn policy(&self, _claim: ClaimId) -> Result<&AcceptancePolicy, ContractError> {
        Err(ContractError::InvalidPolicy)
    }
    fn artifact(&self, reference: ArtifactRef) -> Result<&ArtifactDescriptor, ContractError> {
        self.contract(self.artifact(reference))
    }
    fn declaration(&self, id: ValidationId) -> Result<&Declaration, ContractError> {
        self.contract(self.declaration(id))
    }
...
impl<O: Objects> ResponseArtifacts for Access<'_, O> {
    ...
    fn work(&self, id: ArtifactId) -> Result<&WorkArtifact, ContractError> {
        self.contract(self.work(id))
    }
}
pub(super) struct Policy<'a, 'o, O> {
    pub(super) access: &'a Access<'o, O>,
    pub(super) policy: &'a AcceptancePolicy,
}
impl<O: Objects> evidence::Dependencies for Policy<'_, '_, O> {
    fn policy(&self, claim: ClaimId) -> Result<&AcceptancePolicy, ContractError> {
        if self.policy.claim().object.0 != claim.0
            || self.policy.claim().ledger != self.access.ledger()
        {
    ...
    fn artifact(&self, reference: ArtifactRef) -> Result<&ArtifactDescriptor, ContractError> {
        self.access.contract(self.access.artifact(reference))
    }
    fn declaration(&self, id: ValidationId) -> Result<&Declaration, ContractError> {
        self.access.contract(self.access.declaration(id))
    }
}
impl<O: Objects> read_audit::AuditObjects for Access<'_, O> {
    ...
    fn claim(&self, id: ClaimId, _meter: &Meter) -> Result<&ClaimState, CodecError> {
        self.audit(self.claim(id))
    }
    fn definition(&self, id: ValidationId, _meter: &Meter) -> Result<&Declaration, CodecError> {
        self.audit(self.declaration(id))
    }
    fn result(
        &self,
        key: NativeResultKey,
        _meter: &Meter,
    ) -> Result<(AcceptedResult, PublicationPosition), CodecError> {
        self.audit(self.result(key).map(|(result, at)| (*result, at)))
    }
    fn event(&self, at: PublicationPosition, _meter: &Meter) -> Result<NativeEvent, CodecError> {
        self.audit(self.event(at))
    }
}
...
pub(super) fn heap(row: &Row) -> Result<usize, NativeError> {
    Ok(match row {
        ...
        Row::Definition(value) => value.heap_charge()?,
        Row::Evaluation(value) => value.heap_charge()?,
        Row::Artifact(value) => value.heap_charge()?,
        Row::Accepted(value) => value.heap_charge()?,
        Row::DeliveryResult(value) => value.heap_charge()?,
        Row::MissingResult(value) => value.heap_charge()?,
        Row::Work(value) => value.heap_charge()?,
        Row::Diagnostic(value) => value.heap_charge()?,
        Row::Response(value) => value.heap_charge()?,
        Row::ResultTestament(value) => value.heap_charge()?,
...
fn binding(value: Binding, ledger: LedgerId, id: [u8; 16]) -> Result<(), NativeError> {
    if value.ledger != ledger || value.object.0 != id {
        return Err(invalid());
    }
    Ok(())
}
pub(super) fn check(key: Key, row: &Row, ledger: LedgerId) -> Result<(), NativeError> {
    ...
    match (key, row) {
        (Key::Claim(id), Row::Claim(value)) => {
            binding(value.claim().ok_or_else(invalid)?.binding(), ledger, id.0)?
        }
        (Key::Definition(id), Row::Definition(value)) => {
            binding(value.get().ok_or_else(invalid)?.binding(), ledger, id.0)?
        }
        ...
        (Key::Artifact(id), Row::Artifact(value)) => binding(
            value.get().ok_or_else(invalid)?.descriptor().binding(),
            ledger,
            id.0,
        )?,
        (Key::Work(id), Row::Work(value)) => binding(
            value.get().ok_or_else(invalid)?.state.binding(),
            ledger,
            id.0,
        )?,
        (Key::Response(id), Row::Response(value)) => binding(
            value.get().ok_or_else(invalid)?.identity().binding,
            ledger,
            id.0,
        )?,
        (Key::ResultTestament(id), Row::ResultTestament(value)) => binding(
            value.get().ok_or_else(invalid)?.testament().binding(),
            ledger,
            id.0,
        )?,
        ...
        (Key::Diagnostic(id), Row::Diagnostic(value)) => {
            ...
            if snapshot.ledger != ledger || snapshot.diagnostic.artifact.id != id {
                return Err(invalid());
            }
        }
        (Key::Accepted(key), Row::Accepted(value)) => {
            result_key(key, value.get().ok_or_else(invalid)?.result(), ledger)?
        }
        (Key::DeliveryResult(key), Row::DeliveryResult(value)) => {
            result_key(key, value.get().ok_or_else(invalid)?.result(), ledger)?
        }
        (Key::MissingResult(key), Row::MissingResult(value)) => {
            result_key(key, value.get().ok_or_else(invalid)?.result(), ledger)?
        }
...
fn result_key(
    ...
    if result.ledger() != ledger || NativeResultKey::of(result) != key {
        return Err(invalid());
    }
    Ok(())
}
