...
fn request(id: u128) -> RequestKey {
    RequestKey { principal: ParticipantId::from_u128(1), epoch: RequestEpoch(1), id: RequestId::from_u128(id) }
}
fn inspect(bytes: &[u8]) -> checkpoint::StructuralCheckpoint<'_> {
    checkpoint::StructuralCheckpoint::inspect(bytes, InspectionLimits {
        bytes: bytes.len(), visits: 100_000_000, rows: 100_000, row_bytes: 32 << 20,
    }).unwrap()
}
fn body_range(bytes: &[u8], key: Key) -> Range<usize> {
    let checkpoint = inspect(bytes);
    let row = checkpoint.rows(100_000_000).unwrap().map(Result::unwrap)
        .find(|row| row.key == key).unwrap();
    ...
    let start = (row.body().as_ptr() as usize).checked_sub(bytes.as_ptr() as usize).unwrap();
...
fn checksum(bytes: &mut [u8]) {
    ...
    hash.update(payload); trailer.copy_from_slice(hash.finalize().as_bytes());
}
fn replace_body(bytes: &[u8], key: Key, replacement: &[u8]) -> Vec<u8> {
    ...
    value.extend_from_slice(replacement); value.extend_from_slice(&bytes[range.end..]);
    checksum(&mut value); value
}
fn encode_row(row: &Row, ledger: LedgerId) -> Vec<u8> {
    ...
    rows::value(&mut sink, row, ledger).unwrap(); sink.finish().unwrap(); value
}
...
fn authored_descriptors_identity_indices_and_zero_event_reuse_survive_full_recovery() {
    ...
    let restored = restore(&checkpoint, RangeId(31_405), limits(core.limits), memory.clone(), &content, &BuiltinNativeSchemas).unwrap();
    compare(&core, &restored);
    assert_eq!(restored.native_content_profile(), NativeContentProfile::AuthoredV1);
    assert_eq!(restored.native_claim(ClaimId::from_u128(1)).unwrap().status(), ClaimStatus::Posted);
    for id in [1, 2] {
        ...
        assert!(!std::ptr::eq(new_claim.description().as_ptr(), old_claim.description().as_ptr()));
        ...
        assert_eq!(new_validation.specification_hash(), old_validation.specification_hash());
        ...
        assert_eq!(new_validation.contributed_by(), old_validation.contributed_by());
        assert_eq!(new_validation.policy_revision(), old_validation.policy_revision());
        assert!(matches!(restored.state.rows.get(&Key::ClaimIdentity(new_claim.schema(), new_claim.content_hash())),
            Some(Row::ClaimIdentity(actual)) if *actual == claim));
        assert!(matches!(restored.state.rows.get(&Key::DefinitionIdentity(new_validation.schema(), new_validation.content_hash())),
            Some(Row::DefinitionIdentity(actual)) if *actual == validation));
    }
    for id in [1, 3, 4] {
        assert_eq!(restored.native_creation_result(request(id)).unwrap().entries(),
            core.native_creation_result(request(id)).unwrap().entries());
    }
    assert_eq!(restored.native_creation_result(request(1)).unwrap().entries(),
        restored.native_creation_result(request(3)).unwrap().entries());
    ...
    let claim = restored.native_claim_content(ClaimId::from_u128(1)).unwrap();
    let declaration = restored.native_validation_descriptor(ValidationId::from_u128(1)).unwrap();
    let retry = NativeInput { request: request(3), command: NativeCommand::CreateAuthored { claims: vec![NativeAuthoredProposal {
        content: claim.try_copy(claim.copy_charge().unwrap()).unwrap(),
        declarations: vec![declaration.try_copy(declaration.retained_bytes().unwrap()).unwrap()],
        max_responses: 4, scope_limits: ScopeLimits { scopes: 0, roots: 0, children: 0 }, owner: None,
    }] } };
    assert!(matches!(restored.prepare_native(NativeContext { principal: Principal::Actor(request(3).principal), logical_time: 0 }, retry, &[]).unwrap(),
...
fn altered_authored_body_profile_identity_and_creation_mapping_refuse_and_refund() {
    ...
    let content = store(directory.path());
    let original_budget = core.native_budget();
    let descriptor = core.native_validation_descriptor(ValidationId::from_u128(1)).unwrap();
    let legacy = descriptor.declaration();
    let legacy_row = Row::Definition(OwnedDeclaration::new(legacy.try_copy(legacy.retained_bytes().unwrap()).unwrap()).unwrap());
    let mut mappings = core.native_creation_result(request(3)).unwrap().entries().to_vec();
    ...
    let altered_mapping = NativeCreationResult::from_owned(mappings, 2, checks, mapping_bytes).unwrap();
    ...
    for defect in 0..5 {
        let changed = match defect {
            0 => {
                ...
                let description = core.native_claim_content(ClaimId::from_u128(1)).unwrap().description().as_bytes();
                let at = body.windows(description.len()).position(|value| value == description).unwrap();
                body[at] = b'X';
                replace_body(&bytes, key, &body)
            }
            1 => {
                let mut changed = bytes.clone();
                // Version 1 fixed frame: eight magic bytes, u16 version, profile.
                changed[10] = 0; checksum(&mut changed); changed
            }
            2 => replace_body(&bytes, Key::Definition(ValidationId::from_u128(1)), &encode_row(&legacy_row, core.state.ledger)),
            3 => replace_body(&bytes, Key::ClaimIdentity(1, core.native_claim_content(ClaimId::from_u128(1)).unwrap().content_hash()),
                &ClaimId::from_u128(2).0),
            _ => replace_body(&bytes, Key::CreationResult(request(3).into()), &encode_row(&mapping_row, core.state.ledger)),
        };
        ...
        let memory = budget();
        let before = memory.stats().used;
        let result = restore(&checkpoint, RangeId(31_406), limits(core.limits), memory.clone(), &content, &BuiltinNativeSchemas);
        assert!(result.is_err(), "authored corruption {defect} was accepted");
        drop(result);
        assert_eq!(memory.stats().used, before, "authored corruption {defect} leaked recovery funding");
        assert_eq!(core.native_budget(), original_budget);
    }
}
