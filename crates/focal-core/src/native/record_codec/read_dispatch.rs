...
/// Raw claim bodies come from the original authenticated checkpoint, permitting
/// policy restoration before dependent response rows and full claims exist.
pub(super) trait Objects {
    ...
    fn raw_claim(&self, id: ClaimId, meter: &Meter) -> Result<&[u8], NativeError>;
    fn artifact_origin(&self, id: ArtifactId, meter: &Meter) -> Result<ArtifactOrigin, NativeError>;
}
...
fn add(a: usize, b: usize) -> Result<usize, NativeError> {
    a.checked_add(b).ok_or(NativeError::Capacity("recovery dispatch work"))
}
...
fn parse<'a, T>(body: &'a [u8], meter: &Meter,
    read: impl FnOnce(&mut Cursor<'a>) -> Result<T, NativeError>) -> Result<T, NativeError> {
...
fn quoted(heap_bytes: usize, workspace_bytes: usize, build: Option<(Quote, usize)>) -> Result<Quote, NativeError> {
    let quote = Quote { heap_bytes, workspace_bytes };
    if let Some((expected, allowance)) = build {
        if quote != expected || heap_bytes > allowance { return Err(NativeError::Capacity("recovery row reservation")); }
    }
    Ok(quote)
}
...
fn inspect<P>(context: &Context<'_, impl Objects, impl evidence::Custody>,
...
fn build_evidence(context: &Context<'_, impl Objects, impl evidence::Custody>,
    quote: evidence::Quote, build: impl FnOnce(usize, usize) -> Result<Row, NativeError>) -> Result<Row, NativeError> {
...
pub(super) fn prepare<O: Objects, C: evidence::Custody>(row: &EncodedRow<'_>, context: &Context<'_, O, C>) -> Result<Quote, NativeError> {
...
pub(super) fn with_build<O: Objects, C: evidence::Custody, T>(row: &EncodedRow<'_>, context: &Context<'_, O, C>,
    expected: Quote, retained_allowance: usize,
    finish: impl FnOnce(Row, usize) -> Result<T, NativeError>) -> Result<T, NativeError> {
    let access = Access::new(context.objects, context.lookup);
    ...
    if actual > quote.heap_bytes || actual > retained_allowance { return Err(NativeError::Capacity("recovery actual heap")); }
    finish(value, actual)
}

fn raw_claim<'a, O: Objects>(id: ClaimId, access: &'a Access<'_, O>, parsing: &Meter) -> Result<read_claim::Input<'a>, NativeError> {
    let body = access.raw_claim(id)?;
    let input = parse(body, parsing, |c| read_claim::Input::read(c).map_err(evidence::codec))?;
    if input.fields.binding.ledger != access.ledger() || input.fields.binding.object.0 != id.0
        || input.acceptance.ledger != input.fields.binding.ledger || input.acceptance.object != input.fields.binding.object
        || input.acceptance.content != input.fields.binding.content || input.acceptance_issuer != input.fields.issuer {
        return Err(invalid());
    }
    Ok(input)
}
/// A policy is a temporary immutable dependency, never a synthetic ClaimState.
/// The scratch reservation outlives the built policy and every scoped consumer.
fn with_policy<O: Objects, C: evidence::Custody, T>(claim: ClaimId, context: &Context<'_, O, C>, access: &Access<'_, O>,
    consume: impl FnOnce(&aggregation::AcceptancePolicy, usize) -> Result<T, NativeError>) -> Result<T, NativeError> {
    let input = raw_claim(claim, access, context.parsing)?;
    let source = input.acceptance_source(access, context.source);
    let plan = context.model.model(|visits| {
        let plan = aggregation::AcceptancePolicy::prepare_source(input.acceptance, input.acceptance_issuer,
            &source, context.limits.acceptance, visits)?;
        let used = plan.inspection_visits();
        Ok((plan, used))
    })?;
    let headers = plan.construction_heap_allocations().checked_mul(crate::native::prepare::ALLOCATION)
        .ok_or(NativeError::Capacity("recovery policy allocation count"))?;
    let workspace_bytes = add(plan.construction_heap_bytes(), headers)?;
    let _workspace = context.workspace.reserve(BudgetKind::Recovery, context.workspace_lane, workspace_bytes)?;
...
fn process<O: Objects, C: evidence::Custody>(encoded: &EncodedRow<'_>, context: &Context<'_, O, C>,
    access: &Access<'_, O>, build: Option<(Quote, usize)>) -> Result<(Quote, Option<Row>), NativeError> {
    debit(context.model, 16)?;
    if encoded.deleted() || encoded.key == Key::End { return Err(invalid()); }
    ...
    macro_rules! evidence_plan {
        ($plan:expr, $quote:expr, $workspace:expr) => {{
            ...
            let row = if build.is_some() { Some(build_evidence(context, value_quote,
                |bytes, visits| plan.build(bytes, visits))?) } else { None };
            Ok((quote, row))
        }};
    }
    match encoded.key {
        Key::Claim(id) => {
            let input = parse(body, context.parsing, |c| read_claim::Input::read(c).map_err(evidence::codec))?;
            if input.fields.binding.ledger != ledger || input.fields.binding.object.0 != id.0 { return Err(invalid()); }
            let limits = read_claim::Limits { native: context.limits.native, acceptance: context.limits.acceptance };
            let value_quote = input.quote(access, limits, context.source, context.model)?;
            let quote = quoted(value_quote.heap_bytes, 0, build)?;
            let row = if build.is_some() { Some(input.build(access, limits, context.source, context.model, quote.heap_bytes)?.0) } else { None };
            Ok((quote, row))
        }
        Key::Evaluation(key) => {
            let input = parse(body, context.parsing, |c| read_evaluation::Input::read(c).map_err(evidence::codec))?;
            ...
            let row = if build.is_some() { debit(context.model, 1)?; Some(plan.build(quote.heap_bytes)?.0) } else { None };
            Ok((quote, row))
        }
        Key::ResultTestament(id) => {
            let input = parse(body, context.parsing, |c| read_audit::AuditBody::read(c).map_err(evidence::codec))?;
            if input.snapshot.binding.object.0 != id.0 || input.snapshot.binding.ledger != ledger { return Err(invalid()); }
            if input.members.count > context.limits.native.evaluations_per_claim
                || input.results.count > context.limits.native.results
                || input.snapshot.cohort.result_capacity > u64::try_from(context.limits.native.results)
            ...
            let quote = quoted(value_quote.retained_bytes, value_quote.workspace_bytes, build)?;
            let row = if build.is_some() {
                let _workspace = context.workspace.reserve(BudgetKind::Recovery, context.workspace_lane, quote.workspace_bytes)?;
                let (value, _) = input.build_with_meters(access, value_quote,
                    context.source, context.model).map_err(evidence::codec)?;
        ...
        Key::Artifact(id) => {
            ...
            if fields.ledger != ledger || fields.id != id || origin.binding.revision != focal_model::ObjectRevision(1)
                || origin.binding.ledger != ledger || origin.binding.object.0 != id.0
                || origin.position.sequence.0 == 0 || origin.position.sequence > context.objects.prefix() {
                return Err(invalid());
            }
            let (plan, value_quote) = inspect(context, |model, source| {
                ...
                let quote = plan.quote(); Ok((plan, quote))
            })?;
            let quote = quoted(value_quote.heap_bytes, 0, build)?;
            let row = if build.is_some() { Some(build_evidence(context, value_quote,
                |bytes, visits| plan.build(bytes, visits, context.custody))?) } else { None };
            if let Some(Row::Artifact(value)) = &row {
                let descriptor = value.get().ok_or_else(invalid)?.descriptor();
                if descriptor.content_hash() != origin.binding.content { return Err(ContractError::ContentConflict.into()); }
            }
            Ok((quote, row))
        }
        Key::Definition(_) => {
            let mut input = parse(body, context.parsing, evidence::definition)?;
            let issuer = raw_claim(input.claim(), access, context.parsing)?.fields.issuer;
            let (plan, value_quote) = inspect(context, |model, source| {
                let plan = input.prepare(issuer, context.limits.declaration, context.limits.validation, model, source)?;
                let quote = plan.quote(); Ok((plan, quote))
            })?;
        ...
        Key::ClaimContent(_) => {
            ...
            let parsed = parse(body, context.parsing, |c| evidence::claim_content(c, offered));
            let mut input = match parsed {
                Ok(input) => { debit(context.source, input.parse_source_visits())?; input }
                Err(error) => { debit(context.source, offered)?; return Err(error); }
            };
            let (plan, value_quote) = inspect(context, |model, source| {
        ...
        Key::Work(_) => {
            ...
            let evidence::ScalarInput::Work(snapshot, _) = input else { return Err(invalid()); };
        ...
        Key::Diagnostic(_) | Key::Accepted(_) | Key::DeliveryResult(_) | Key::MissingResult(_) => {
            let input = parse(body, context.parsing, |c| match encoded.key {
                Key::Diagnostic(_) => evidence::diagnostic(c), Key::Accepted(_) => evidence::accepted(c),
                Key::DeliveryResult(_) => evidence::delivery(c), Key::MissingResult(_) => evidence::missing(c),
                _ => Err(invalid()),
            })?;
            let (plan, value_quote) = inspect(context, |model, _source| {
        ...
        Key::Response(_) => {
            let input = parse(body, context.parsing, |c| evidence::response(c,
                context.limits.native.response_summary_bytes, context.source.remaining()))?;
            with_policy(input.fields().identity.claim, context, access, |policy, workspace| {
        ...
        Key::CreationResult(_) => {
            let input = parse(body, context.parsing, |c| evidence::creation(c, context.source.remaining()))?;
            let (plan, value_quote) = inspect(context, |model, _source| {
        ...
        Key::Event(..) => {
            let plan = parse(body, context.parsing, |c| read_rows::EventPlan::read(encoded.key, c, ledger))?;
            let quote = quoted(plan.heap_bytes(), 0, build)?;
            let row = if build.is_some() {
                let visits = plan.build_visits(); debit(context.model, visits)?;
                Some(plan.build(quote.heap_bytes, visits)?.0)
            } else { None };
            Ok((quote, row))
        }
        Key::IncomingHead(_) | Key::IncomingLink(..) | Key::Monitor(_) | Key::MonitorHead(_)
        | Key::MonitorLink(..) | Key::Meta | Key::ArtifactIdentity(_) | Key::Receipt(_)
        | Key::Cycle(_) | Key::RetiredCycleHead(_) | Key::RetiredCycle(_) | Key::WorkSlot(..)
        | Key::ClaimResultTestament(_) | Key::Outcome(_) | Key::ClaimIdentity(..) | Key::DefinitionIdentity(..) => {
            let row = parse(body, context.parsing, |c| read_rows::read_fixed(encoded.key, c, ledger))?.ok_or_else(invalid)?;
