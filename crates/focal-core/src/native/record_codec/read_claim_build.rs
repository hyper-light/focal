...
fn add(a: usize, b: usize) -> Result<usize, NativeError> {
    a.checked_add(b)
        .ok_or(NativeError::Capacity("claim recovery charge"))
}
fn nested(heap: usize, allocations: usize) -> Result<usize, NativeError> {
    add(
        ...
        allocations
            ...
            .ok_or(NativeError::Capacity("claim recovery allocations"))?,
    )
}
fn vector<T>(count: usize) -> Result<usize, NativeError> {
    nested(
        count
            ...
            .ok_or(NativeError::Capacity("claim recovery vector"))?,
        usize::from(count != 0),
    )
}
fn fits(actual: usize, maximum: usize) -> Result<(), NativeError> {
    if actual > maximum {
        Err(NativeError::Capacity("claim recovery allowance"))
    } else {
        Ok(())
    }
}
fn collect<T>(
    mut source: impl Iterator<Item = Result<T, ContractError>>,
    count: usize,
    ...
    if values.capacity() != count {
        return Err(NativeError::Capacity("claim recovery allocation"));
    }
    for _ in 0..count {
        values.push(source.next().ok_or(ContractError::InvalidManifest)??);
    }
    match source.next() {
        None => Ok(values),
...
fn process<D: Objects + ?Sized>(
    ...
    if fields.responses
        ...
        || fields.max_responses == 0
        || input.registrations.max_rows > limits.native.evaluations_per_claim
    ...
    // The immutable definition retains its original revision while the claim
    // lifecycle advances. Compare its identity at that original revision; the
    // model hydration boundary checks the current revision against the origin.
    input.lineage.check(&Binding {
        revision: input.lineage.revision,
        ..fields.binding
    })?;
    Binding {
        revision: fields.binding.revision,
        ..input.acceptance
    }
    .check(&fields.binding)?;
    if input.acceptance_issuer != fields.issuer {
        return Err(ContractError::InvalidPolicy.into());
    }
    model.budget(|visits| {
        graph::Declaration::check_sorted_values(
            Values::new(input.obligations, source, obligation),
            input.obligations.count,
            limits.native.plan_edges,
            visits,
        )
    })?;
    model.budget(|visits| {
        succession::Lineage::check_sorted_values(
            input.lineage,
            &input.cause,
            Values::new(input.corrections, source, correction),
            input.corrections.count,
            limits.native.plan_edges,
            visits,
        )
    })?;
    ...
    let graph_heap = add(
        vector::<graph::Obligation>(input.obligations.count)?,
        vector::<succession::Correction>(input.corrections.count)?,
    )?;
    let policy_heap = nested(
        acceptance.construction_heap_bytes(),
        acceptance.construction_heap_allocations(),
    )?;
    let scope_heap = nested(
        scopes.construction_heap_bytes(),
        scopes.construction_heap_allocations(),
    )?;
    let history_heap = add(
        claim::ClaimState::hydration_response_heap_charge_v1(fields.responses)?,
        aggregation::RegistrationSet::hydration_heap_charge_v1(input.registrations.rows)?,
    )?;
    let heap = add(
        OwnedClaim::container_charge(),
        add(
            add(graph_heap, policy_heap)?,
            add(scope_heap, history_heap)?,
        )?,
    )?;
    let quote = ClaimQuote { heap_bytes: heap };
    let Some(allowance) = build else {
    ...
    let obligations = collect(
        Values::new(input.obligations, source, obligation),
        input.obligations.count,
    )?;
    let graph = model.budget(|visits| {
        graph::Declaration::from_owned_sorted(obligations, limits.native.plan_edges, visits)
    })?;
    let corrections = collect(
        Values::new(input.corrections, source, correction),
        input.corrections.count,
    )?;
    let lineage = model.budget(|visits| {
    ...
    let definition = claim::ClaimDefinition {
        binding: input.lineage,
        issuer: fields.issuer,
        subject: fields.subject,
        deadline: fields.deadline,
        max_responses: fields.max_responses,
        created: fields.created,
        graph,
        lineage,
        acceptance,
        scope_limits: input.scopes.fields.limits,
    };
    let responses = input.response_source(objects, source);
    let plan = model.model(|visits| {
        let plan = claim::ClaimState::prepare_hydration_v1(
            definition, fields, &responses, scopes, visits,
        )?;
        let used = plan.inspection_visits();
        Ok((plan, used))
    })?;
    ...
    let registrations = model.model(|visits| {
        let plan = aggregation::RegistrationSet::prepare_hydration_v1(
            &state,
            input.registrations,
            &members,
            limits.native.evaluations_per_claim,
            visits,
        )?;
        let used = plan.inspection_visits();
        Ok((plan, used))
