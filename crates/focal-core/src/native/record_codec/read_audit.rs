...
use super::{bytes::{Cursor, Error}, read_fields as f, read_source::{Meter, Span, Values}};
use crate::native::{audit_bundle::{OwnedResultTestament, RecoveredCoordinates},
    NativeAuditPublication, NativeError, NativeEvent, NativeFact, NativeResultKey};
use focal_model::lifecycle::{Binding, ContractError, aggregation::PublicationPosition,
    audit::{self, AuditMemberSnapshotV1, ResultTestament, ResultTestamentState},
    claim::ClaimState, validation::{self, AcceptedResult, AcceptedResultSnapshotV1, Declaration}};
...
pub(super) trait AuditObjects {
    ...
    fn result(&self, key: NativeResultKey, meter: &Meter) -> Result<(AcceptedResult, PublicationPosition), Error>;
...
fn add(a: usize, b: usize) -> Result<usize, Error> { a.checked_add(b).ok_or(Error::Capacity) }
fn mul(a: usize, b: usize) -> Result<usize, Error> { a.checked_mul(b).ok_or(Error::Capacity) }
fn buffer<T>(count: usize) -> Result<usize, Error> {
    add(mul(count, size_of::<T>())?, if count == 0 { 0 } else { ALLOCATION })
}
fn model(error: ContractError) -> Error {
    match error { ContractError::Capacity => Error::Capacity, _ => Error::InvalidTag("audit model") }
}
fn native(error: NativeError) -> Error {
    match error {
        NativeError::Contract(error) => model(error), NativeError::Capacity(_) => Error::Capacity,
        NativeError::Memory(_) => Error::Allocation, _ => Error::InvalidTag("native audit"),
    }
}
fn reserve<T>(count: usize) -> Result<Vec<T>, Error> {
    ...
    if values.capacity() > count { return Err(Error::Allocation); }
    Ok(values)
}
fn push<T>(values: &mut Vec<T>, value: T) -> Result<(), Error> {
    if values.len() >= values.capacity() { return Err(Error::Capacity); }
    values.push(value);
    Ok(())
}
fn publication(c: &mut Cursor<'_>) -> Result<NativeAuditPublication, Error> {
    Ok(NativeAuditPublication { key: f::result_key(c)?, position: f::position(c)? })
}
fn as_u64(value: usize) -> Result<u64, Error> { u64::try_from(value).map_err(|_| Error::Capacity) }
...
impl<'a> AuditBody<'a> {
    ...
    pub(super) fn read(c: &mut Cursor<'a>) -> Result<Self, Error> {
        ...
        let coordinates = RecoveredCoordinates { generated, captured_at: f::sequence(c)?,
            generated_at: f::position(c)?, posted_at: f::optional_position(c)? };
        let publications = Span::read_with(c, publication)?;
        Ok(Self { generated, snapshot: audit::ResultTestamentSnapshotV1 { binding, state,
            cohort: audit::AuditCohortSnapshotV1 { claim, issuer, sequence,
                members: as_u64(members.count)?, results: as_u64(results.count)?, result_capacity } },
            members, results, coordinates, publications })
    }
    pub(super) fn quote(&self) -> Result<AuditQuote, Error> {
        let capacity = usize::try_from(self.snapshot.cohort.result_capacity).map_err(|_| Error::Capacity)?;
        if self.results.count > capacity || self.publications.count != self.results.count {
            return Err(Error::InvalidTag("audit result counts"));
        }
        let workspace_bytes = add(buffer::<AuditMemberSnapshotV1>(self.members.count)?,
            add(buffer::<AcceptedResultSnapshotV1>(self.results.count)?, buffer::<&Declaration>(self.members.count)?)?)?;
        let retained_bytes = add(OwnedResultTestament::container_charge(),
            add(buffer::<audit::AuditMember>(self.members.count)?,
                add(buffer::<AcceptedResult>(capacity)?, buffer::<NativeAuditPublication>(self.publications.count)?)?)?)?;
        Ok(AuditQuote { workspace_bytes, retained_bytes })
    }
    ...
    pub(super) fn build(&self, objects: &impl AuditObjects, allowance: AuditQuote, meter: &Meter)
        -> Result<(OwnedResultTestament, usize), Error>
    {
        self.build_with_meters(objects, allowance, meter, meter)
    }
    pub(super) fn build_with_meters(&self, objects: &impl AuditObjects, allowance: AuditQuote,
        source: &Meter, meter: &Meter) -> Result<(OwnedResultTestament, usize), Error>
    {
        meter.charge(64)?;
        let quote = self.quote()?;
        if quote.workspace_bytes > allowance.workspace_bytes || quote.retained_bytes > allowance.retained_bytes {
            return Err(Error::Capacity);
        }
        if self.generated != self.coordinates.generated || self.generated.revision != ObjectRevision(1)
            || self.coordinates.captured_at < self.snapshot.cohort.sequence
            || self.coordinates.generated_at.sequence > objects.prefix()
            || self.coordinates.posted_at.is_some_and(|at| at.sequence > objects.prefix())
        ...
        let actual_workspace = add(buffer::<AuditMemberSnapshotV1>(members.capacity())?,
            add(buffer::<&Declaration>(declarations.capacity())?, buffer::<AcceptedResultSnapshotV1>(results.capacity())?)?)?;
        if actual_workspace > quote.workspace_bytes { return Err(Error::Capacity); }
        ...
        let capacity = usize::try_from(self.snapshot.cohort.result_capacity).map_err(|_| Error::Capacity)?;
        let model_bytes = add(size_of::<ResultTestament>(),
            add(buffer::<audit::AuditMember>(members.len())?, buffer::<AcceptedResult>(capacity)?)?)?;
        meter.charge(model_visits)?;
        let plan = ResultTestament::prepare_hydration_v1(claim, self.generated, self.snapshot,
            &members, &results, &declarations, audit::Limits { evaluations: members.len(), results: capacity },
            model_bytes, model_visits).map_err(model)?;
        if plan.construction_charge() > model_bytes { return Err(Error::Capacity); }
        ...
        for publication in Values::new(self.publications, source, publication) {
            ...
            let fact = match result.phase() {
                validation::Phase::Delivery => NativeFact::Delivery { key: publication.key },
                validation::Phase::MissingTarget => NativeFact::Missing { key: publication.key },
                validation::Phase::Programmatic | validation::Phase::Quality => NativeFact::Accepted { key: publication.key },
            };
            if event.sequence != position.sequence || event.ordinal != position.ordinal || event.fact != fact {
        ...
        for result in testament.results() {
            ...
            if actual != *result { return Err(Error::InvalidTag("audit result body")); }
        }
        let visits = OwnedResultTestament::recovery_visits(testament.cohort().members().len(),
            testament.results().len(), publications.len()).map_err(native)?;
        meter.charge(visits)?;
        OwnedResultTestament::recover(testament, publications, self.coordinates, quote.retained_bytes, visits).map_err(native)
    }
}

fn generation_events(body: &AuditBody<'_>, objects: &impl AuditObjects, meter: &Meter) -> Result<(), Error> {
    meter.charge(64)?;
    ...
    if generated.sequence != body.coordinates.generated_at.sequence
        ...
        || generated.fact != (NativeFact::ResultTestament { claim, before: None,
            after: body.generated, state: ResultTestamentState::Generated })
    ...
    match (body.snapshot.state, body.coordinates.posted_at) {
        ...
        (ResultTestamentState::Posted, Some(position)) => {
            ...
            if posted.sequence != position.sequence || posted.ordinal != position.ordinal
                || posted.fact != (NativeFact::ResultTestament { claim, before: Some(body.generated),
                    after: body.snapshot.binding, state: ResultTestamentState::Posted })
