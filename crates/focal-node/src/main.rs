...
use cli::cluster::ClusterCommand;
use focal_model::*;
use focal_node::{
    config::Settings,
...
struct Args {
    /// Load node and connection defaults from a YAML configuration file.
    #[arg(long, global = true)]
    config: Option<PathBuf>,
    /// Use this directory for node identity, durable ledger and local client state.
    #[arg(long, global = true)]
...
enum Commands {
    ...
    Context {
        #[command(subcommand)]
        command: cli::context::ContextCommand,
    },
    ...
    Identity,
    /// Inspect released operation and built-in schema contracts without opening a ledger.
    Schema {
        #[command(subcommand)]
        command: cli::discovery::SchemaCommand,
    },
...
fn execute(args: Args) -> Result<()> {
    let args = match args.command {
        ...
        Commands::Request(cli::RequestArgs {
            file: None,
            command: Some(cli::RequestCommand::Check { file }),
...
fn run(runtime: &tokio::runtime::Runtime, args: Args) -> Result<()> {
    ...
    match args.command {
        ...
        Commands::Status => {
            cli::status(runtime, &settings, args.client_context.as_deref()).map_err(Into::into)
        }
        Commands::Request(command) => Ok(cli::request(
            runtime,
            &settings,
            command,
            args.client_context.as_deref(),
        )?),
        Commands::Manual(command) => Ok(cli::run(
            runtime,
            &settings,
            *command,
            args.client_context.as_deref(),
...
fn output_response(reply: ResponseEnvelope) -> Result<()> {
    let failed = match &reply.result {
        Response::Error(error) => Some(cli::CliError::Client(focal_client::ClientError::Access(
            error.clone(),
        ))),
        Response::Submitted(MutationReply::Domain(outcome @ DomainOutcome::Refuse { .. })) => {
            Some(cli::CliError::Domain(outcome.clone()))
        }
        Response::Submitted(MutationReply::Pending(_)) => Some(cli::CliError::Unconfirmed),
        _ => None,
    };
    print_json(&reply)?;
