...
fn read(fixture: &mut Fixture, id: u128) -> oneshot::Receiver<OwnedResponse> {
    ...
    assert!(
        bound < 16 * 1024,
        "scalar reply must not reserve a full megabyte frame"
    );
    let charge = owner
        .budget
...
#[test]
fn summary_waiters_release_allowance_on_cancel_deadline_route_change_and_delivery_drop() {
    ...
    assert!(matches!(reply.envelope().result, Response::Summary(_)));
    let held = fixture.owners[0].budget.stats().used;
    drop(reply);
    assert!(fixture.owners[0].budget.stats().used < held);
