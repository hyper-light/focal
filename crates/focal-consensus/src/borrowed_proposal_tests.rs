...
fn borrowed_record_survives_capacity_refusal_then_commits_exact_original_bytes() {
    ...
    let source_charge = budget.reserve(BudgetKind::Pending, BudgetLane::Completion, 1024).unwrap();
    ...
    let pressure = budget.reserve(
        BudgetKind::Payload, BudgetLane::Completion, budget.stats().limit - budget.stats().used,
    ).unwrap();
    let full = budget.stats();
    assert!(matches!(node.propose_borrowed_in(&record, BudgetLane::Completion), Err(ConsensusError::Capacity)));
    ...
    node.propose_borrowed_in(&record, BudgetLane::Completion).unwrap();
...
fn borrowed_record_refuses_wrong_authority_and_invalid_sizes_without_new_raft_charge() {
    ...
    assert!(matches!(node.propose_borrowed_in(b"retained", BudgetLane::Completion), Err(ConsensusError::NotLeader { .. })));
    assert_eq!(budget.stats(), before);
    node.campaign().unwrap();
    drop(node.drain().unwrap());
    let before = budget.stats();
    for value in [&[][..], &[1; 1025][..]] {
        assert!(matches!(node.propose_borrowed_in(value, BudgetLane::Completion), Err(ConsensusError::Capacity)));
        assert_eq!(budget.stats(), before);
    }
...
fn funded_checkpoint_transfers_source_lifetime_without_losing_durable_prefix() {
    ...
    node.campaign().unwrap();
    node.drain().unwrap();
    node.propose_borrowed_in(b"before", BudgetLane::Completion).unwrap();
    let index = node.drain().unwrap().applied_index;
    let old = budget.stats();
    let insufficient = budget.reserve(BudgetKind::Recovery, BudgetLane::Completion, 1).unwrap().commit();
    assert!(matches!(node.begin_checkpoint_funded(index, vec![7; 1024], insufficient), Err(ConsensusError::Capacity)));
    assert_eq!(budget.stats(), old);
    assert!(!node.checkpoint_pending());

    let allocation = budget.reserve(BudgetKind::Recovery, BudgetLane::Completion, 1024 + 4 * size_of::<usize>()).unwrap().commit();
    let before = budget.stats().by_kind[BudgetKind::Recovery as usize];
    node.begin_checkpoint_funded(index, vec![8; 1024], allocation).unwrap();
    assert!(node.checkpoint_pending());
    ...
    assert_eq!(budget.stats().by_kind[BudgetKind::Recovery as usize], before - 1024 - 4 * size_of::<usize>());
    node.finish_checkpoint().unwrap();
    node.propose_borrowed_in(b"after", BudgetLane::Completion).unwrap();
    node.drain().unwrap();
    drop(node);
    assert_eq!(budget.stats().used, 0);
