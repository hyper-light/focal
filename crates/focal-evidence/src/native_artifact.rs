...
use focal_model::lifecycle::artifact_descriptor::{ArtifactDescriptor, ContentPointer, PayloadSpec};
...
impl NativeLocalCustody {
    pub fn payload(&self) -> ContentPointer { self.payload }
    pub fn check(&self, request: RequestKey, artifact: &ArtifactDescriptor) -> Result<(), NativeEvidenceError> {
        if self.request != request || self.descriptor != artifact.intent_fingerprint()
    ...
    pub fn local_revision(&self) -> u64 { 1 }
}
...
impl std::fmt::Debug for VerifiedNativeArtifact {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("VerifiedNativeArtifact").field("custody", &self.custody).finish_non_exhaustive()
    }
}
impl VerifiedNativeArtifact {
    pub fn custody(&self) -> NativeLocalCustody { self.custody }
    pub fn check(&self, request: RequestKey, descriptor: &ArtifactDescriptor) -> Result<(), NativeEvidenceError> {
        self.custody.check(request, descriptor)
    }
    pub fn retained_bytes(&self) -> usize { self.allocation.bytes() }

}

fn add(a: usize, b: usize) -> Result<usize, ContentError> { a.checked_add(b).ok_or(ContentError::Capacity) }
fn pointer(reference: &ContentRef) -> ContentPointer {
    ContentPointer { domain: reference.domain, root: reference.root, length: reference.length, class: reference.class }
}
fn reference(pointer: ContentPointer) -> ContentRef {
    ContentRef { domain: pointer.domain, root: pointer.root, length: pointer.length, class: pointer.class }
}
...
impl ContentStore {
    ...
    pub fn verify_native_artifact(
        &mut self, request: RequestKey, descriptor: &ArtifactDescriptor,
        inline_domain: ContentDomainId, budget: &MemoryBudget, schemas: &impl NativeSchemaVerifier,
    ) -> Result<VerifiedNativeArtifact, NativeEvidenceError> {
        self.check()?;
        if request.principal != descriptor.producer() || request.principal.is_zero()
            || request.id.is_zero() || request.epoch.0 == 0 || inline_domain.is_zero()
        { return Err(NativeEvidenceError::WrongRequest); }
        let maximum = schemas.maximum_bytes(descriptor.schema_hash())?;
        if maximum > MAX_TRANSFER_MANIFEST_BYTES { return Err(ContentError::Capacity.into()); }
        let retained = std::mem::size_of::<VerifiedNativeArtifact>();
        ...
        let mut allocation = budget.reserve(BudgetKind::Payload, BudgetLane::Completion, add(retained, add(STORE_WORKSPACE, maximum)?)?)?.commit();
        let payload = match descriptor.payload() {
            PayloadSpec::Inline(bytes) => {
                if bytes.len() > maximum { return Err(ContentError::Capacity.into()); }
        ...
        let custody = NativeLocalCustody { request, descriptor: descriptor.intent_fingerprint(), payload };
        allocation.shrink_to(retained)?;
    ...
    fn seal_native_inline(&mut self, domain: ContentDomainId, bytes: &[u8]) -> Result<ContentRef, ContentError> {
        self.check()?;
        let result = self.seal_native_inline_inner(domain, bytes);
        self.mark_failure(&result);
        result
    }
    fn seal_native_inline_inner(&self, domain: ContentDomainId, bytes: &[u8]) -> Result<ContentRef, ContentError> {
        let length = u64::try_from(bytes.len()).map_err(|_| ContentError::Capacity)?;
        if length > self.limits.max_content_bytes || bytes.len() > MAX_TRANSFER_MANIFEST_BYTES { return Err(ContentError::Capacity); }
        let count = bytes.len().checked_add(self.limits.chunk_bytes.checked_sub(1).ok_or(ContentError::Capacity)?).ok_or(ContentError::Capacity)?
            .checked_div(self.limits.chunk_bytes).ok_or(ContentError::Capacity)?;
        if count.checked_mul(40).is_none_or(|n| n > self.limits.max_manifest_bytes) { return Err(ContentError::Capacity); }
        let mut chunks = Vec::new();
        chunks.try_reserve_exact(count).map_err(|_| ContentError::Capacity)?;
        if chunks.capacity().checked_mul(std::mem::size_of::<Chunk>()).is_none_or(|n| n > STORE_WORKSPACE) { return Err(ContentError::Capacity); }
        for block in bytes.chunks(self.limits.chunk_bytes) {
            let hash = ContentHash(*blake3::hash(block).as_bytes());
            chunks.push(Chunk { hash, length: u32::try_from(block.len()).map_err(|_| ContentError::Capacity)? });
        }
        let manifest = Manifest { schema: 1, domain, class: ContentClass::Evidence, length,
            stream_digest: ContentHash(*blake3::hash(bytes).as_bytes()), chunks };
        let encoded_size = postcard::experimental::serialized_size(&manifest)?;
        let size = add(MANIFEST_MAGIC.len(), encoded_size)?;
        if size > self.limits.max_manifest_bytes { return Err(ContentError::Capacity); }
        let mut encoded = Vec::new();
        encoded.try_reserve_exact(size).map_err(|_| ContentError::Capacity)?;
        if encoded.capacity() > self.limits.max_manifest_bytes { return Err(ContentError::Capacity); }
        encoded.resize(size, 0);
        encoded.get_mut(..MANIFEST_MAGIC.len()).ok_or(ContentError::Corrupt)?.copy_from_slice(MANIFEST_MAGIC);
        let written = postcard::to_slice(&manifest, encoded.get_mut(MANIFEST_MAGIC.len()..).ok_or(ContentError::Corrupt)?)?;
        if written.len() != encoded_size { return Err(ContentError::Corrupt); }
        ...
        for (block, chunk) in bytes.chunks(self.limits.chunk_bytes).zip(&manifest.chunks) {
            install_verified_chunk(&directory.join(format!("{}.chunk", chunk.hash)), block, chunk.hash)?;
        }
        atomic_install(&directory.join(format!("{root}.manifest")), &encoded)?;
        Ok(ContentRef { domain, root, length, class: ContentClass::Evidence })
    }
}
