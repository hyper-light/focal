...
/// Tools derived from the shared watch descriptors.
pub(crate) const TOOL_COUNT: usize = operations::WATCH_TOOL_COUNT;
pub(crate) fn append(tools: &mut Vec<Tool>) -> Result<(), ProtocolError> {
    tools
        .try_reserve_exact(TOOL_COUNT)
        .map_err(|_| ProtocolError::Capacity)?;
    for descriptor in operations::watch_descriptors() {
