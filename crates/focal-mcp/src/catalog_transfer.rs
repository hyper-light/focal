...
/// Tools derived from the shared transfer descriptors.
pub(crate) const TOOL_COUNT: usize = operations::TRANSFER_TOOL_COUNT;
pub(crate) fn append(tools: &mut Vec<Tool>) -> Result<(), ProtocolError> {
    tools
        .try_reserve_exact(TOOL_COUNT)
        .map_err(|_| ProtocolError::Capacity)?;
    for descriptor in operations::transfer_descriptors() {
