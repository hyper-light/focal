...
pub fn wire_coverage(operation: &Operation) -> Coverage {
    match operation {
        ...
        Operation::Native { .. } => coverage(
            "native.submit",
            Capability::Actor,
            Exposure::WireAvailable,
            true,
            "Native profile only; the frame's principal, request identity and ledger must equal the authenticated envelope, and the owner proves every role from its committed prefix.",
            "NativeMutationReply: Committed receipt, Pending ticket or closed refusal",
        ),
        Operation::NativeRead(_) => coverage(
            "native.read",
            Capability::Actor,
            Exposure::WireAvailable,
            false,
            "Native profile only; fixed-prefix documents of committed native rows under the requested consistency.",
            "NativeReadPage",
        ),
        Operation::NativeList(_) => coverage(
            "native.list",
            Capability::Actor,
            Exposure::WireAvailable,
            false,
            "Native profile only; one indexed predicate plus residual filtering within the visit allowance and an authenticated cursor.",
            "NativeListPage with explicit visits and continuation",
        ),
        ...
        Operation::PlacementControl { .. } => coverage(
            "peer.placement_control",
            Capability::Node,
            Exposure::InternalOnly,
            true,
            "Active enrolled certificate; bounded partition reads and the sender's own enrollment, load, progress and readiness.",
            "ControlReply",
        ),
        Operation::SessionSign { .. } => coverage(
            "peer.session_sign",
            Capability::Node,
            Exposure::InternalOnly,
            false,
        ...
        Operation::Probe { .. } => coverage(
            ...
            Exposure::InternalOnly,
            false,
            "Active enrolled certificate; a liveness probe or an indirect probe relayed to a peer, answered from published state.",
            "ProbeReply",
        ),
