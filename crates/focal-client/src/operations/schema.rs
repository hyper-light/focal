...
const DEFINITIONS: &str = r##"{
  ...
  "testament":{"type":"object","description":"The current respondent authors this report after work completes or fails. Every non-complete outcome requires a durable, schema-valid error artifact in the exact manifest; the server verifies its hash, receipt and producer. Reporting does not establish claim satisfaction.","additionalProperties":false,"required":["claim","receipt","evidence_set","manifest","summary","confidence","outcome"],"allOf":[{"if":{"properties":{"outcome":{"enum":["partial","refused","impossible","interrupted","failed"]}},"required":["outcome"]},"then":{"properties":{"manifest":{"minItems":1}}}}],"properties":{
...
pub(super) fn input(descriptor: &OperationDescriptor) -> Result<Value, InputError> {
    if let InputKind::Native(kind) = descriptor.input {
        return super::native_schema::input(descriptor, kind);
    }
    if let InputKind::Literal(text) = descriptor.input {
        return serde_json::from_str(text)
            .map_err(|_| InputError::Invalid("released schema literal"));
    }
    ...
    let key = match descriptor.input {
        InputKind::Native(_) | InputKind::Literal(_) => {
            return Err(InputError::Invalid("schema routing"));
        }
...
pub(super) fn output() -> Result<Value, InputError> {
    serde_json::from_str(r##"{
      ...
      "properties":{
        "schema_version":{"enum":[1,2]},"operation_id":{"type":["string","null"],"minLength":1,"maxLength":128},"condition":{"type":"string","minLength":1,"maxLength":64},
        "result":{"oneOf":[
          ...
          {"type":"object","additionalProperties":false,"required":["kind","receipt","created"],"properties":{"kind":{"const":"native"},"receipt":{"type":"object","description":"Typed native receipt: invocation, native prefix position, logical time, operation, intent fingerprint and row counts; validated by focal-wire."},"created":{"type":"array","maxItems":4096,"items":{"type":"object","additionalProperties":false,"required":["kind","id"],"properties":{"kind":{"enum":["Claim","Validation","Receipt","Artifact","Response"]},"id":{"type":"array","minItems":16,"maxItems":16,"items":{"type":"integer","minimum":0,"maximum":255}}}}}}},
          {"type":"object","additionalProperties":false,"required":["kind","page"],"properties":{"kind":{"const":"native_read"},"page":{"type":"object","description":"Typed fixed-prefix native read page: token, native sequence, logical time, documents, continuation and visits; validated by focal-wire."}}},
          {"type":"object","additionalProperties":false,"required":["kind","page"],"properties":{"kind":{"const":"native_list"},"page":{"type":"object","description":"Typed bounded native list page with its opaque continuation cursor; validated by focal-wire."}}},
          {"type":"object","additionalProperties":false,"required":["kind","result"],"properties":{"kind":{"const":"native_wait"},"result":{"type":"object","additionalProperties":false,"required":["condition","until","observation","probes"],"properties":{"condition":{"enum":["Met","Pending","Unmet"]},"until":{"enum":["testament","satisfied","terminal","released"]},"probes":{"type":"integer","minimum":1,"maximum":31},"observation":{"type":"object","description":"Latest native claim observation: read token, claim, status code, revision, local completion and release."}}}}},
          {"type":"object","additionalProperties":false,"required":["kind","refusal"],"properties":{"kind":{"const":"native_refused"},"refusal":{"type":"object","additionalProperties":false,"required":["kind","detail"],"properties":{"kind":{"description":"Closed native refusal category: InvalidInput, Unauthorized, NotFound, Stale{binding}, Conflict, Capacity or Refused(code); validated by focal-wire."},"detail":{"type":"string","maxLength":16384}}}}}
        ]}
      },
      "$defs":{
        ...
        "replica_diagnostics":{"type":"object","additionalProperties":false,"required":["node","cluster","session","group","leader","term","committed_index","applied_index","sequence","pending","authoritative","persistence_pending","checkpoint_pending","compiled_managed_decoder","required_decoder","managed_active","compiled_native_decoder","native_hosted","native_ready","native_active","native_import_pending","native_authoritative"],"properties":{"node":{"type":"integer","minimum":1},"cluster":{"type":"string","pattern":"^[0-9a-f]{32}$"},"session":{"type":"string","pattern":"^[0-9a-f]{32}$"},"group":{"type":"string","pattern":"^[0-9a-f]{32}$"},"leader":{"type":"integer","minimum":0},"term":{"type":"integer","minimum":0},"committed_index":{"type":"integer","minimum":0},"applied_index":{"type":"integer","minimum":0},"sequence":{"type":"integer","minimum":0},"pending":{"type":"integer","minimum":0},"authoritative":{"type":"boolean"},"persistence_pending":{"type":"boolean"},"checkpoint_pending":{"type":"boolean"},"compiled_managed_decoder":{"type":"string","pattern":"^[0-9a-f]{64}$"},"required_decoder":{"type":["string","null"],"pattern":"^[0-9a-f]{64}$"},"managed_active":{"type":"boolean"},"compiled_native_decoder":{"type":"string","pattern":"^[0-9a-f]{64}$"},"native_hosted":{"type":"boolean"},"native_ready":{"type":"boolean"},"native_active":{"type":"boolean"},"native_import_pending":{"type":"boolean"},"native_authoritative":{"type":"boolean"}}},
