...
impl<'a> Checkpoint<'a> {
    pub fn inspect(bytes: &'a [u8], limits: Limits) -> Result<Self, Error> {
        ...
        if cursor.fixed::<6>()? != [0; 6] {
            return Err(Error::Invalid("legacy ancillary state is unsupported"));
        }
        ...
        Ok(Self {
            ...
            core_bytes,
            configuration,
            remaining: Cell::new(cursor.remaining),
