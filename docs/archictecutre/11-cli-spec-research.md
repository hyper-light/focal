...
Implementation follow-up: the source inventory below records the baseline at research time. The local domain CLI, optional-filter listing, durable journals, artifact download and validation-result pagination have since landed; use [the manual guide](../manual-cli.md) for executable commands and [the status record](09-implementation-status.md) for remaining scope. Proposed remote, cluster, MCP and workflow rows remain plans unless explicitly verified there.
