...
Precedence for this proposal is the user's current instruction, then the accepted Focal contracts, then reconciled Hecate specifications, with Sylk supplying concrete lessons. The current instruction is explicit: **Focal is peer-to-peer, not a job system.** The issuer invokes tools/skills in its own environment; a respondent supplies testament/artifacts; the issuer or a designated peer records a verdict. Focal stores and verifies the resulting authority, evidence, fences and lifecycle. It does not launch agents/workers or invoke providers. Evaluation by another agent is an ordinary claim/testament exchange. Hecate harness execution mechanisms below are source evidence, not a requirement to install that harness in Focal. In particular:
...
The source audit also requires **four coordinated object lifecycles**. Sylk's
[Artifacts and Validations §§2, 5, 7, 11](../../../sylk/docs/ARTIFACTS_AND_VALIDATIONS.md)
and [testament lifecycle §§3.3, 5, 6](../../../sylk/docs/CLAIMS_AND_TESTAMENTS_LIFECYCLE.md)
define per-artifact receipt/attachment/validation, per-validation phase outcomes,
and per-testament posting/receipt/validation. Triggered parent transitions are
atomic with their child facts. Focal currently has complete claim status plus
thin testament/artifact/validation lifecycle metadata and run records; the
[gap matrix in 02](02-domain-and-lifecycle.md#35-four-coordinated-lifecycle-families-implementation-gap)
records the missing model work. Immutable evidence and a claim-level validation
status alone do not close that gap. The initial D-03 single-close profile is not
equivalent to Sylk's multiple posted testament aggregation.
...
Everything in this section is the proposed Focal policy satisfying the current user instruction. It is encoded as pinned claim requirements plus a recovery-safe workflow owned by the participating agents. Focal enforces legal records and transitions; it neither runs that workflow nor becomes its author.
...
|---|---|
| Required proof passes | The authorized issuer/evaluator submits the result; Focal applies the normal validation and dependency rules. |
| Required proof is absent | Record `Incomplete`; an authorized participant authors the policy-required corrective work identifying the missing proof. |
| Supplied proof violates the quality bar | Record `Fail`; an authorized participant authors corrective work citing the failing evidence and requirement. |
...
The authorized participant uses a stable decision/issuance identity binding the original ledger, challenge, terminal testament/run, policy revision and author. It persists the resulting claim identities before acknowledging the evidence stream position used for that decision. Re-delivery must locate the same corrective work. A passive subscriber or Focal daemon cannot acquire authorship by observing a failure. A failed corrective returns to its participant for bounded judgment and escalation; it cannot manufacture an unbounded tree of corrections.
...
Use `Awaits`/terminality when the parent needs the result even if it failed. Use `DependsOn`/satisfaction when successful work is a hard prerequisite. If a consult continuation is waiting for satisfaction, it cannot be released by tool completion or by a posted response. The designated peer can evaluate independently without releasing the parent's blocked obligation. Its participant owns any bounded continuation metadata and model execution. Focal stores the monitor and dependency facts, not a parked model worker.
...
flowchart LR
  ...
  D --> R[Participant-owned continuation and tools]
  R --> A
  A --> P[Peer evaluator via ordinary claim and testament]
  P --> C
```
...
The released catalog uses individual stable operation names from [the shared registry](../../crates/focal-client/src/operations/catalog.rs), with typed schemas and server-side standing checks. It includes peer lifecycle mutations, requirement/result inspection, traversal and summary, plus `request.reserve`, `request.pending`, `request.inspect`, `request.retry`, `request.acknowledge` and `request.seal`. Optional transfer, durable watch and local administration backends add their own typed tools; [the executable guide](../mcp.md) describes discovery. Designated evaluators submit fenced verdicts as authenticated participants. This does not grant generic Runtime authority. Local operator tools use the actual selected physical node's authenticated administrative socket; a remote participant context does not inherit those tools. The broader façades below group workflow requirements; the façade names themselves are not registered tools.
...
| `focal_validation` | `get`, `list`, `results`, `inspect`, `evaluate` | Bounded requirement/run/result inspection with optional claim, evaluator, kind, phase and mode. Actual run/attempt/verdict pages are separate from immutable requirement filtering. Submit `RecordFencedValidationVerdict` through the narrow authenticated peer path. The agent supplies evidence and a typed verdict, never a raw status edit. Fixed-prefix validation context, issuer begin/complete and designated participant verdict submission are implemented; trusted child-cause peer-evaluation claims and independent target lifecycle migration remain required. External evaluation stays participant-owned. |
...
- `list` filters are all optional for every object family. Omitting them returns a bounded page from the selected authorized ledger. Filters compose with AND; repeated values within a filter use a documented set rule. Unsupported combinations reject explicitly. Pagination binds principal/scope, filters, sort order, fixed read prefix, visited position, and expiry; even an empty filtered page carries continuation when scan work remains. Artifact-by-testament queries follow that immutable manifest at the same prefix. Validation results distinguish the requirement, run, attempt, verdict, and proof. These bounded queries are implemented for the predicates documented in [the current interface contract](19-cli-mcp-implementation.md); new independent lifecycle/result-state predicates still require their authoritative model and indexes.
...
The core already treats receipt validation separately, checks pending attempts and authority on verdict recording, and performs deterministic phase transitions. Participants execute their own validating tools, then submit evidence and fenced verdicts. The existing `focal-runtime` library is an optional participant-owned embedding helper with a bounded local executor; it is not the Focal daemon's required scheduler or agent launcher. Replay executes neither models nor validators. [Validation reducer](../../crates/focal-core/src/validation.rs#L95), [runtime execution contract](../../crates/focal-runtime/README.md#L37)

### Trusted lifecycle operations require an explicit participant seam

The released peer profile exposes testament receipt, validation begin and deterministic completion through narrowly authorized issuer intents, and fenced results through the designated evaluator. The owner verifies actual standing, receipt/manifest fences and effective revisions before using an individual legacy command. It does not install a scheduler or grant generic Runtime authority. Timer firing, receipt adoption, scope release and placement publication retain separate trusted owners; root enrollment and membership administration use the distinct operator interface. No caller may supply the authority bit that executes a privileged transition. The admission bridge does not replace the independent lifecycle storage upgrade. Exact capability assignments are defined by code, not by a skill description. [Capability mapping](../../crates/focal-wire/src/auth.rs#L150), [peer implementation](19-cli-mcp-implementation.md#peer-operations).
...
Use the existing node/tenant/session budget hierarchy for decoded input, prepared command, queued continuation, artifact chunks, and encoded response. Preserve completion capacity for verdicts, ACKs, deadline handling, and failure evidence. A yielding consult releases the participant's execution resources while retaining its bounded durable obligation. Cancellation releases the participant's local wait interest; it neither silently cancels a committed claim nor erases an admitted effect. [Ownership policy](10-ownership-and-failure-policy.md), [runtime bounds and cancellation](../../crates/focal-runtime/README.md)
...
### C. Expose peer-owned validation and evidence submission

Provide authenticated issuer/evaluator operations that bind the exact claim, testament manifest, validation contract, attempt and receipt fence. The issuer invokes its validating tool/skill externally and supplies the resulting evidence. When another agent evaluates, the issuer addresses an ordinary claim to that peer and consumes its testament/artifacts. Focal verifies the submitting principal and evidence before recording the verdict and deriving lifecycle. It does not create a worker registry, allocate model execution or launch a provider. Existing Runtime-only commands need a narrow participant authorization seam, not blanket Runtime grants.
...
Provide participant-owned bounded continuation support backed by committed state and durable cursors; the participant decides when to resume its own work. Compile `consult` and `challenge` through the same claim-generation path as ordinary work. Capture exact target references, resolve the target through authorized routing, and preserve the parent cause. The present ingress supplies a trusted root cause and verifies claim content against it; changing a DTO to `Cause::Claim` is insufficient. Add the planned narrow child-claim admission seam that verifies parent existence, actor/receipt, relationship/rank, and committed policy before the owner constructs trusted parentage. Let the participating evaluator recover its own evaluation independently of any blocked parent. Provide bounded active-exchange lookup and nonblocking tracking. [Current cause check](../../crates/focal-wire/src/auth.rs#L198), [P17.12 parentage work](13-cli-and-agent-implementation-plan.md#3-p17--typed-application-operations-and-bounded-queries)
...
Expose an exact, fenced decision/issuance operation for the authorized participant that judges a terminal challenge failure. That participant authors the actual corrective claims and their causal references. A durable subscriber may report the observed failure, but cannot autonomously author work or grant itself corrective standing. Add the consult instruction/template path that produces targeted follow-up consultations. Preserve generic explicit corrective work when a consult discovers a separate defect; do not hide a global action-type switch in the reducer.

Acceptance: the authorized participant's decision on absent/failed challenge proof creates one recoverable corrective lineage across duplicate events and restart; observation alone creates no work. Infrastructure errors identify infrastructure, not fabricated responder fault. An unsatisfactory consult normally produces a linked follow-up consultation. Failed corrective work cannot recursively spawn an unbounded tree. Old testaments and verdicts remain unchanged.
