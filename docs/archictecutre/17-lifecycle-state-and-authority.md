...
L1's remaining owner-integration obligations keep that gate open. These are
...
| Response testament | Immutable closing manifest for one work cycle, authored by the respondent; distinct from the claim's aggregate state |
...
| Received or Progressed | Authorized exceptional closing failure with real durable diagnostics | TestamentGenerationFailed; never synthesize successfully generated bytes |
...
The executable successor contract restricts final post failure to the issuer,
receipt-boundary failure to the subject, and exceptional closing failure to the
current receipt holder. Each requires an actual typed durable diagnostic and the
matching nonterminal phase. This does not expose the historical broad Runtime-only
commands to every Actor or authorize a caller-selected aggregate status.
...
| Prior state | Writer and guard | Next state / atomic effects |
|---|---|---|
| Absent | Current respondent closes a bounded exact manifest of previously Generated/Received work artifacts | Generated + every listed artifact Attached; freeze content/bindings and advance the first response's claim to TestamentGenerated |
...
Partial, Refused, Impossible, Interrupted and Failed authored outcomes remain
uniform testimony, not exceptions to receipt or proof checking. A request whose
...
activation path. It prepares checked values and transitions; the state owner must
still persist and publish them under L2–L5. It is not a second live ledger.
...
| [Acceptance manifest and registry](../../crates/focal-model/src/lifecycle/acceptance.rs) | Owned complete immutable declarations, exact target/content/generation/receipt registration, separate increment and audit seals, Required Admission and Increment gates, complete pure Receipt proof set |
...
reports requested bytes and actual retained capacities, but the live owner's
budget reservations, durable representation and publication remain work under
[the storage plan](18-lifecycle-storage-upgrade.md#61-concrete-successor-owner-integration).

The remaining owner obligations keep L1 open: integrate every transition through
one complete effective-state registry, enforce full lineage checks on every claim
creation path, and derive owned-tree cancellation/fencing consequences together.
Posting standing and logical-time inputs remain trusted effective owner views;
ingress must never accept them as participant permission flags. Start and graph
release now consume checked proof tokens rather than caller-selected booleans.
...
capacity. The count-bounded contract allocations do not qualify those storage,
proof-byte, history-retention or publication budgets.
