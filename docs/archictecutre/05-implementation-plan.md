...
- [x] P10.1 Batch committed entries into versioned epochs. Epoch-sizing changes are explicit deterministic control inputs or implementation choices proven not to affect externally observable output. (2026-09-09: a delivery's run of consecutive records is one batch, bounded by `max_batch`; batch size never changes the rows, [25](25-parallel-materialization-and-ranges.md) §2)
- [x] P10.2 Derive complete read/write footprints over objects, indexes, adjacency, aggregate counters, deadlines, identities and monitor state. Track actual reads/writes; no unchecked pointer escape bypasses tracking. (2026-09-09: write keys from the record, object affinity per key, every actual read traced through the base-rows seam)
- [x] P10.3 Build RAW/WAR/WAW predecessor edges in log order, deduplicating edges and excluding self-edges for read-modify-write on the same key. Bound graph allocation by admitted epoch bytes. (2026-09-09: edges by affinity and exact key in log order, bounded by `max_edges`, none for entry-local rows)
- [x] P10.4 Execute ready tasks over an immutable base plus per-entry multiversion output. Preserve the values earlier-index reads require. A maximum-writer-index cell alone is insufficient. (2026-09-09: each stage reads the latest completed earlier writer per key, else the base; earlier versions stay available to earlier readers)
- [x] P10.5 At a deterministic validation barrier audit complete actual conflicts. On incomplete footprints discard the entire affected speculative epoch/suffix and recompute it serially, including downstream results. No timing-dependent partial overlay is published. (2026-09-09: the barrier compares every traced read with the complete writer set; a stale read discards the record and its suffix, redone serially)
- [x] P10.6 Publish only a contiguous successful prefix with immutable deltas in sequence/ordinal order. Reclaim speculative memory even on cancellation/crash. Retain the serial execution path as correctness reference, not legacy behavior. (2026-09-09: staged records install and publish in order; a refusal stops the batch at its record and the delivery resumes there)
- [x] P10.7 (2026-09-09: independent and chained shapes measured at one and four workers, [25](25-parallel-materialization-and-ranges.md) §2; the serial path stays the default at one worker and the node picks up to four) Measure zero-conflict, dense-conflict, long-chain and high-fan-in workloads. Automatically use serial execution when overhead exceeds useful parallel work without changing semantics.
...
- [x] P11.1 Implement a single ordered range map for graph storage and apply. Affinity groups and secondary-index buckets have explicit keys; claim-owned state remains colocated where practical. (Storage order [25 §3](25-parallel-materialization-and-ranges.md), range groups and the generic map [25 §4](25-parallel-materialization-and-ranges.md), 2026-09-09.)
- [x] P11.2 Make every active range consume/skip the common session prefix, tracking applied completeness even when an entry does not touch it. Select bounded verified replica readers for each range. (Every member advances per publication by a fragment, empty when untouched, so the group has one prefix; a read pins every member at it [25 §4](25-parallel-materialization-and-ranges.md), 2026-09-09. Verified remote readers per range come with movement, P11.5.)
- [ ] P11.3 Implement deterministic cross-range read exchange and write batches keyed by session sequence, epoch and transaction identity. The log commits the decision once; retrying materialization cannot create an independent transaction decision. (Movement records are session decisions keyed by control ordinal and native prefix with genesis-attested commit proofs, [25 §6](25-parallel-materialization-and-ranges.md), 2026-09-09; the voters still materialize every member, so cross-range reads at admission remain.)
- [ ] P11.4 Publish a stable prefix only after all required range results are complete. A reader pins the route map and consistent prefix; new leaders reconstruct the publication lower bound before serving. (Every holder materializes the whole session and a learner serves only the members it holds, [25 §7](25-parallel-materialization-and-ranges.md), 2026-09-09; steering readers to holders through the directory remains.)
- [ ] P11.5 Implement logged split/move intent, checkpoint seed, tail catch-up, stop barrier, fenced activation, directory update and delayed cleanup. Directory CAS alone must never authorize both source and destination. (Logged split and merge intents apply on every replica as `FOCALRG1` records, [25 §4](25-parallel-materialization-and-ranges.md), 2026-09-09; checkpoint seeds are chunked, sealed while streaming and pulled by a copy before its placement activates, [25 §5](25-parallel-materialization-and-ranges.md), 2026-09-09; the logged intent, stop barrier, readiness and seal proofs, fenced activation and delayed cleanup are session-log records every replica applies alike, with admission fenced on the moving member between barrier and activation, [25 §6](25-parallel-materialization-and-ranges.md), 2026-09-09; a holder is a learner with native hosting whose readiness, seal and progress facts travel over its own connection and are verified against the authority's member digest, and the leader's controller carries an operator's move through every step, [25 §7](25-parallel-materialization-and-ranges.md), 2026-09-09; the controller resumes any step from the committed map after a crash cut at each of the seven sites through the real binary, claims leadership of sessions with work, and publishes the holders to the directory as a monotone, placement-checked fact, [25 §9](25-parallel-materialization-and-ranges.md), 2026-09-10; steering readers to holders remains.)
- [ ] P11.6 Implement idempotent resume at every transfer step, snapshot and cursor token translation, range merge, balancing hysteresis and fair transfer budgets. User sessions retain their IDs and claim history. (Range merge and balancing hysteresis: the leader's controller splits a member past twice the target rows for three observations and merges adjacent members under a quarter of it, as ordinary layout records, [25 §8](25-parallel-materialization-and-ranges.md), 2026-09-10; a transfer resumes from its committed record on every replica, [25 §6](25-parallel-materialization-and-ranges.md); a listing's continuation is shown to survive a move, [25 §9](25-parallel-materialization-and-ranges.md), 2026-09-10; fair transfer budgets remain.)
...
**Progress (2026-09-10).** Custody receipts per verified copy and the obligation a validation phase reads before it begins ([26 §1](26-custody-archive-retention-and-restore.md), R8.2); the disk envelope every durable owner promises its bytes to came with R6.5 ([24 §10](24-placement-execution-and-fleet-control.md)); schema and validator identities across upgrades ([26 §2](26-custody-archive-retention-and-restore.md), R8.3); the log's retirement boundary by checkpoint cadence and the retention floor with the archive's checkpointed report ([26 §3](26-custody-archive-retention-and-restore.md), R8.4a); retirement of terminal, released families to content-addressed bundles under custody through a committed record, behind typed continuations, with the archive agent, the operator's retention and archive reads and the real-binary journey ([26 §4](26-custody-archive-retention-and-restore.md), R8.4b, decision F49) — P12.2's retirement, custody and continuations are delivered; the collector of P12.5 with its roots, quarantine and reopen reconciliation, and the bounded per-step pacing of P12.6 ([26 §5](26-custody-archive-retention-and-restore.md), R8.5, decision F50) are delivered; coherent backups at a declared prefix with source verification ([26 §6](26-custody-archive-retention-and-restore.md), R8.6 first step, decision F51) and whole-session restore with verification before serving and the recovery incarnation ([26 §6](26-custody-archive-retention-and-restore.md), R8.6 second step, decision F52) are delivered; the operator's storage view and the sustained-bounds and corruption evidence of R8's close gate ([26 §7](26-custody-archive-retention-and-restore.md), decision F53) are delivered; searchable identity/relationship indexes over bundles, the receipt floors of P12.3, pacing by work credits under load, and the cold proof lookup of P12.7 beyond bundle verification remain.
